...
import { AlertTriangle, Info, AlertCircle, Wrench, Check, X } from "lucide-react";
import type { ReviewFinding, FindingAction } from "@/types";
...
const severityConfig = {
  critical: { icon: AlertCircle, color: "text-red-600 dark:text-red-400", bg: "bg-red-50 dark:bg-red-900/20", badge: "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300" },
  warning: { icon: AlertTriangle, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-50 dark:bg-amber-900/20", badge: "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300" },
  info: { icon: Info, color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-50 dark:bg-blue-900/20", badge: "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300" },
};
...
const validityConfig = {
  ...
  undecided: { label: "Undecided", color: "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300" },
};
        ...
        <div className="flex-1 min-w-0">
          ...
          {finding.suggestion && (
            <div className="mt-1.5 rounded bg-zinc-100 dark:bg-zinc-800 p-1.5 font-mono text-[11px]">
              <span className="text-emerald-600 dark:text-emerald-400 font-sans text-[10px] uppercase tracking-wider">suggestion</span>
              <pre className="mt-0.5 whitespace-pre-wrap text-zinc-600 dark:text-zinc-400">{finding.suggestion}</pre>
            </div>
          )}
          ...
          <div className="flex items-center gap-1.5 mt-2">
            {action ? (
              <span className="text-[10px] text-muted-foreground">
                {action.action === "fix" ? "Requested fix" : action.action === "accept" ? "Accepted" : "Dismissed"}
              </span>
            ) : (
              <>
                <Button
