...
function generateSummary(activities: ToolActivity[]): string {
  ...
  const parts = Object.entries(counts).map(([label, count]) => {
    ...
    const pastVerb = verb.replace(/ing$/, "ed").replace("Running", "Ran").replace("Searching", "Searched");
...
export function ActivityGroup({ activities, summaryText, isActive }: ActivityGroupProps) {
  ...
  return (
    <div className={`my-2 rounded-lg border-l-2 ${isActive ? "border-l-emerald-500" : "border-l-zinc-200 dark:border-l-zinc-700"}`}>
      <button
