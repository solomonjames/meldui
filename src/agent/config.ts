...
export function buildSystemPromptAppend(config: AgentConfig): string {
  ...
  parts.push(`- **meldui_request_approval(ticket_id, summary, items)** — Ask user to review specific items before continuing.`);
  ...
  parts.push(`\nWhen you produce a deliverable (spec section, investigation report, etc.), write it to the appropriate ticket field using meldui_write_section. When done with a step, call meldui_step_complete.`);
